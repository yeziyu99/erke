    ...
    <div class="erke_content">
      <el-collapse v-model="activeNames">
        <el-collapse-item
          :title="itemt.title"
          :name="index"
          v-for="(itemt, index) in erkeData"
          :key="index"
        >
          <template slot="title">
            <!-- 一层的标题 -->
            <el-avatar v-if="itemt.img" class="iconImg" :src="itemt.img"> </el-avatar>
            <i v-if="!itemt.img" :class="itemt.icon + ' iconImg header-icon'"></i>
            <span class="font-weight-700 font-size-16">
              {{ itemt.title }}
            </span>
            <span
              class="mg-lt-10 font-size-12 font-weight-700 color62657A"
              v-if="!vertical"
              >{{ itemt.conter }}</span
            >
          </template>
          ...
          >
            <el-descriptions-item v-for="(item, index) in itemt.children" :key="index">
              <template slot="label">
                ...
                <span
                  v-if="item.text"
                  style="color: #000; font-size: 16px; font-weight: 700"
                >
                  {{ item.title }}
                </span>
                <span v-if="!item.text">
              ...
              <span v-for="(items, index) in item.children" :key="index">
                ...
                <el-tag
                  size="small"
                  class="mg-rg-5 pointer"
                  v-if="items.tag && items.herf"
                  @click="externalJumpFn(items.herf)"
                  >{{ items.tag }}</el-tag
                >
                <!-- 纯展示 -->
                <el-tag
          ...
          <el-timeline v-if="itemt.timeline == 'one'">
            <el-timeline-item
              ...
              :timestamp="items.time"
              placement="top"
              v-for="(items, index) in itemt.children"
              :key="index"
            >
              <!-- <el-card> -->
              <h4>{{ items.title }}</h4>
            ...
            <span>
              ...
              >
                <el-tag
                  size="small"
                  type="success"
                  @click="copyText('success', 'Alba_1015')"
                  class="pointer"
                >
