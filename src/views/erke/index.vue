<template>
    <div class="erke background_linear" :style="{ minHeight: clientHeight + 'px' }" ref="page">
        <div class="erke_content">
            <el-collapse v-model="activeNames">
                <el-collapse-item :title="itemt.title" :name="index" v-for="(itemt, index) in erkeData" :key="index">
                    <template slot="title">
                        ...
                        <span class="mg-lt-10 font-size-12 font-weight-700 color62657A" v-if="!vertical">{{ itemt.conter }}</span>
                    </template>
                    <!-- 二层表格系列 -->
                    <el-descriptions class="margin-top" :column="1" border v-if="itemt.timeline != 'one'" :direction="vertical">
                        <el-descriptions-item v-for="(item, index) in itemt.children" :key="index">
                            ...
                            <span v-for="(items, index) in item.children" :key="index">
                                ...
                                <!-- 纯展示 -->
                                <el-tag size="small" class="mg-rg-5 pointer"
                                    v-if="items.tag && (!items.herf) && (!items.type)">
                                    {{ items.tag }}</el-tag>
                                <!-- 特殊提示带 复制可加可不加 type是success才可复制 -->
                                <el-tooltip class="item" effect="dark" :content="items.tooltip" placement="bottom-start">
                                    <el-tag size="small" :type="items.type" @click="copyText(items.type)"
                                        class="mg-rg-5 pointer" v-if="items.tag && (!items.herf) && items.type">{{ items.tag
                                        }}</el-tag>
                                </el-tooltip>
                            </span>
                    ...
                    <el-descriptions-item>
                        ...
                        <span>
                            <el-tooltip class="item" effect="dark" content="点击复制" placement="bottom-start">
                                <el-tag size="small" type="success" @click="copyText('success', 'Alba_1015')"
                                    class="pointer">
    ...
    mounted() {
        ...
        window.onresize = () => {
            return (() => {
                ...
                if (that.$refs.page) {
                    that.$refs.page.style.minHeight = that.clientHeight - 100 + 'px';
                }
            })()
        }
    },
